...
const SHOW_LEGACY_PACKAGES_BLOCK = false; // ← включишь обратно: true

// ---- типы -----

type PackageType = '' | 'basic' | 'standard' | 'comfort' | 'premium' | 'custom';
...
interface CoffinVariant {
  ...
  title: string;
  subtitle?: string;
  image: string; // путь до картинки в /public
}

const COFFIN_VARIANTS: CoffinVariant[] = [
  {
    id: 'pine-white',
    title: 'Гроб из сосны',
    subtitle: 'Белая обивка внутри',
    image: '/coffins/pine-white.jpg',
  },
  {
    id: 'pine-beige',
    title: 'Гроб из сосны',
    subtitle: 'Бежевая обивка внутри',
    image: '/coffins/pine-beige.jpg',
  },
  {
    id: 'oak-white',
    title: 'Гроб из дуба',
    subtitle: 'Белая обивка внутри',
    image: '/coffins/oak-white.jpg',
  },
];
...
const additionalServices: AdditionalService[] = [
  ...
  { id: 'fresh-flowers', name: 'Живая флористика', price: 12000, description: 'Композиции, гирлянды', icon: Package },
...
export function PackagesSection({ formData, onUpdateFormData }: PackagesSectionProps) {
  ...
  const filteredFeatures = showDifferencesOnly
    ? comparisonFeatures.filter((feature) => !(feature.standard === feature.comfort && feature.comfort === feature.premium))
    : comparisonFeatures;

  const renderComparisonValue = (value: boolean | string) => {
    if (typeof value === 'boolean') {
      return value ? <Check className="h-5 w-5 text-green-600 mx-auto" /> : <X className="h-5 w-5 text-gray-300 mx-auto" />;
    }
    ...
    <div className="inline-flex items-center gap-3 p-1 bg-white rounded-full border border-gray-200">
      <TabsList className="bg-transparent p-0 h-auto gap-0">
        ...
        >
          ...
          {/* ===== ВКЛАДКА ГОТОВЫХ ПАКЕТОВ (LEGACY) ===== */}
          {SHOW_LEGACY_PACKAGES_BLOCK && (
            <TabsContent value="packages" className="space-y-8">
              <div className="text-center">
                <p className="text-gray-600 mb-6">Выберите подходящий тариф</p>
              </div>

              {/* Переключатели типа и вида */}
                ...
                <div className="inline-flex items-center gap-2 p-1 bg-white rounded-full border border-gray-200">
                  <button
                    ...
                    className={cn(
                      ...
                      ceremonyType === 'burial' ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-100',
                    )}
                  >
                    Захоронение
                  </button>
                  <button
                    ...
                    className={cn(
                      'rounded-full px-6 py-2 transition-all text-sm',
                      ceremonyType === 'cremation' ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-100',
                    )}
                  >
                    Кремация
                  </button>
                  ...
                  {PACKAGES.map((pkg) => (
                    <div
                      ...
                      className={cn(
                        ...
                        formData.packageType === pkg.id && 'ring-2 ring-gray-900 ring-offset-2',
                      )}
                    >
                      <div className="p-8 flex-1 flex flex-col">
                        ...
                        {formData.packageType === pkg.id && (
                          <div className="mt-6 border-t border-gray-100 pt-6">
                            <h4 className="mb-4 text-sm font-medium text-gray-900">Выберите внешний вид гроба</h4>

                            <div className="grid gap-4 sm:grid-cols-3">
                              {COFFIN_VARIANTS.map((variant) => {
                                const isActive = formData.coffinVariant === variant.id;
                                return (
                                  <button
                                    key={variant.id}
                                    type="button"
                                    onClick={() => onUpdateFormData('coffinVariant', variant.id)}
                                    className={cn(
                                      'flex flex-col rounded-2xl border bg-white p-3 text-left transition-all hover:shadow-md',
                                      isActive && 'border-gray-900 shadow-gray-900/10',
                                    )}
                                  >
                                    <div className="aspect-[4/3] overflow-hidden rounded-xl bg-gray-100">
                                      <img src={variant.image} alt={variant.title} className="h-full w-full object-cover" />
                                    </div>
                                    <div className="mt-2 text-sm font-medium text-gray-900">{variant.title}</div>
                                    {variant.subtitle && <div className="text-xs text-gray-500">{variant.subtitle}</div>}
                                  </button>
                                );
                              })}
                            </div>
                  ...
                  <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        />
                        <label htmlFor="differences-table" className="text-sm cursor-pointer select-none">
                          Только отличия
                        </label>
                      </div>
                    </div>
                            ...
                            <tr className="border-b-2 border-gray-200">
                              ...
                              <th className="text-center p-3 sm:p-4 bg-gray-50 min-w-[110px]">
                                <div className="space-y-1">
                                  ...
                                  <div className="text-xs sm:text-sm text-gray-600">{PACKAGES[0].price.toLocaleString('ru-RU')} ₽</div>
                                </div>
                              </th>
                              <th className="text-center p-3 sm:p-4 bg-gray-100 min-w-[110px]">
                                <div className="space-y-1">
                                  <div className="text-sm sm:text-base">Комфорт</div>
                                  <div className="text-xs sm:text-sm text-gray-600">{PACKAGES[1].price.toLocaleString('ru-RU')} ₽</div>
                                </div>
                              </th>
                              <th className="text-center p-3 sm:p-4 bg-gray-50 last:rounded-tr-2xl min-w-[110px]">
                                <div className="space-y-1">
                                  ...
                                  <div className="text-xs sm:text-sm text-gray-600">{PACKAGES[2].price.toLocaleString('ru-RU')} ₽</div>
                                </div>
                              </th>
                            </tr>
                          </thead>
                              ...
                              <Fragment key={category}>
                                ...
                                {features.map((feature, index) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      <td className={cn('p-3 sm:p-4 text-xs sm:text-sm', isLastInTable && 'rounded-bl-2xl')}>
                                        {feature.name}
                                      </td>
                                      <td className="p-3 sm:p-4 text-center">{renderComparisonValue(feature.standard)}</td>
                                      <td className="p-3 sm:p-4 text-center bg-gray-50/50">
                                        {renderComparisonValue(feature.comfort)}
                                      </td>
                                      <td className={cn('p-3 sm:p-4 text-center', isLastInTable && 'rounded-br-2xl')}>
          ...
          {/* ===== ВКЛАДКА "СОБРАТЬ ПАКЕТ" ===== */}
          <TabsContent value="custom" className="space-y-6">
            <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-6">
              <p className="text-gray-700">
                Выберите дополнительные услуги для индивидуального пакета. Базовая стоимость —{' '}
                ...
                return (
                  <Card
                    ...
                    className={cn(
                      ...
                      isSelected ? 'ring-2 ring-gray-900 bg-gray-50' : 'hover:ring-1 hover:ring-gray-300',
                    )}
                  >
